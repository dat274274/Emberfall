export default class Player extends Phaser.Physics.Arcade.Sprite {
    constructor(scene, x, y, savedLevel = 1, savedExp = 0) {
        ...
        scene.input.on('pointerdown', (pointer) => {
            ...
            if (pointer.leftButtonDown()) {
                ...
                // Hiệu ứng marker (Sử dụng tọa độ local để scale không bị trượt)
                this.targetMarker.clear();
    ...
    update(cursors, wasd, time, spaceBar, skillKey, skillKey2) {
        ...
        if (this.isTornadoing || this.isDashing || this.isAttacking) {
            this.setVelocity(0);
            return;
        }
    ...
    die() {
        ...
        this.play(`${this.skinPrefix}_death`);
        this.once('animationcomplete', () => {
            ...
            const ui = this.scene.add.container(cam.midPoint.x, cam.midPoint.y).setDepth(10000);
            
            const bg = this.scene.add.rectangle(0, 0, cam.width / cam.zoom, cam.height / cam.zoom, 0x000000, 0.7);
            ui.add(bg);
            
            const overText = this.scene.add.text(0, -50, 'GAME OVER', {
                fontSize: '48px', fontFamily: 'Impact', color: '#ff0000',
                stroke: '#000', strokeThickness: 4
            }).setOrigin(0.5);
            ui.add(overText);
            
            const btnBg = this.scene.add.rectangle(0, 40, 180, 50, 0x444444, 1).setInteractive();
            btnBg.setStrokeStyle(2, 0xffffff);
            const btnText = this.scene.add.text(0, 40, '🔄 CHƠI LẠI', {
                fontSize: '20px', fontFamily: 'Verdana', color: '#ffffff', fontStyle: 'bold'
            }).setOrigin(0.5);
            ui.add(btnBg);
            ui.add(btnText);
            ...
            this.scene.events.on('prerender', () => {
                ui.setPosition(cam.midPoint.x, cam.midPoint.y);
            });
        });
    }
}